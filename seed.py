...
def example_data():
    ...
    for ride in rides:
        time = ride.start_timestamp.time()
        ride.start_timestamp = datetime.combine(today,time) + timedelta(days = day_offset[str(ride.ride_id)])
        db.session.commit()


    ## Use this if you need to make a new .sql file
